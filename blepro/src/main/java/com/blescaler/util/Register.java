...
public class Register {
    ...
    public byte[] BeginRead(int reg_addr, int reg_num) {
        ...
        byte out[] = {0x20, 0x3, (byte) ((u_reg_addr >> 8) & 0xff), (byte) (u_reg_addr & 0xFF), (byte) ((u_reg_num >> 8) & 0xff), (byte) (u_reg_num & 0xFF), 0};

        byte result = CheckSum.calc(out, 0, out.length - 1);
        out[out.length - 1] = result;
...
//		byte[] out  = new byte[length+2];
        ...
        byte result = CheckSum.calc(out, 0, length);
        out[length] = result;
...
//		out[length+0] = (byte)(crc16&0xFF);
    ...
    private static boolean  readRegisters(String addr, int reg_addr,int num)
    {
        //设备地址 1byte
        //命令类型 0x3
        //起始寄存器地址 reg_addr
        ...
        byte result = CheckSum.calc(buffer, 0, buffer.length-1);
        buffer[6] = result;
